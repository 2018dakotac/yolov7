...
modelName = input("Enter name of yolov7 (standard form) weights (include .pt)")
device = select_device('0', batch_size=1)
# model trained by cfg/training/*.yaml
ckpt = torch.load(modelName, map_location=device)
# reparameterized model in cfg/deploy/*.yaml
model = Model('cfg/deploy/yolov7custom1class.yaml', ch=3, nc=1).to(device)
...
torch.save(ckpt, 'cfg/deploy/deployed_' + modelName)